...
#define RX_TIMEOUT           400  // fast enough for button, long enough for LoRa preamble
...
uint8_t  pendingSeq      = 0;   // fixed for all retries of same packet
...
void transmitPacketWithSeq(uint8_t* outbuf, size_t* outlen,
                            uint8_t dst, uint8_t type,
  ...
  if(outbuf){ memcpy(outbuf,buf,total); *outlen=total; }
}

// ─── Transmit new packet — increments mySeq, returns seq used ─
uint8_t transmitNewPacket(uint8_t dst, uint8_t type,
                           const char* plain, bool needsAck){
  uint8_t seq=mySeq++;
  transmitPacketWithSeq(nullptr,nullptr,dst,type,plain,needsAck,seq);
  return seq;
}
...
void startDataSend(const char* payload){
  ...
  pendingRetry=0;
  pendingSeq=mySeq++;   // assign seq once, reuse for all retries
  transmitPacketWithSeq(nullptr,nullptr,
                        PARTNER_ID,PKT_DATA,payload,true,pendingSeq);
...
void startAlertSend(const char* msg){
  ...
  pendingSeq=mySeq++;   // assign seq once, reuse for all retries
  transmitPacketWithSeq(nullptr,nullptr,
                        PARTNER_ID,PKT_ALERT,msg,true,pendingSeq);
...
void checkTxState(){
  ...
  // Deadline expired — do one final listen before giving up
  ...
  if(pendingRetry<maxR){
    // Retransmit with SAME seq number
    ...
    transmitPacketWithSeq(nullptr,nullptr,PARTNER_ID,tp,pl,true,pendingSeq);
