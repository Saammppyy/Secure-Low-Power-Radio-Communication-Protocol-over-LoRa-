// ============================================================
//  NODE B — Collision avoidance update
//  Changes:
...
//  4. Duplicate packets needing ACK get ACK resent (not flagged as attack)
//  5. ATTACK BLOCKED only shown for explicit simulated replay
// ============================================================
...
void sendAck(uint8_t dst,uint8_t seq){
  ...
  txCooldownUntil=millis()+TX_COOLDOWN_MS;
}
...
void checkTxState(){
  ...
  if(pendingRetry<MSG_MAX_RETRIES){
    // Randomized jitter — desyncs both nodes' retry timing
    delay(random(50,250));
...
void processPacket(uint8_t* buf,size_t len){
  ...
  // Dedup — but resend ACK for legitimate retries
  if(seq==lastSeenSeq){
  ...
  switch(type){
    ...
    case PKT_HEARTBEAT:
      ...
      // Only respond with heartbeat if we're idle (avoid collision)
